...
/** Brand names available for a category, e.g. ["Dasani", "Refill"] for water. */
export function getBrands(category: InventoryCategory): string[] {
  return getCatalog(category).map((b) => b.brand);
}

/** Find a single catalog item by id within a category — used to build inventory/supplier entries from the shared catalog. */
export function findCatalogItem(category: InventoryCategory, id: string): CatalogItem | undefined {
