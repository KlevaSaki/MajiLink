...
interface Props {
  rating: number;
  totalReviews: number;
  /** Omitted entirely when there's no real per-star data to show —
   *  nothing in the schema writes this yet, so a zeroed breakdown would
   *  read as five real bars of "no reviews" rather than "not tracked." */
...
export default function RatingCard({ rating, totalReviews, breakdown }: Props) {
  ...
  return (
    <div className="bg-white border border-[#D6D3D1] rounded-2xl p-4 flex items-center gap-5">
          ...
          {[1, 2, 3, 4, 5].map((n) => (
            <Star
              ...
              className={`w-3.5 h-3.5 ${
                totalReviews > 0 && n <= Math.round(rating)
      ...
      {/* Breakdown — only when there's real data to show */}
      {bars && (
        <>
